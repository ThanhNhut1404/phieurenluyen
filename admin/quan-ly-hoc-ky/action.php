<?php
    ...
    function hocky_store_old_input($context, $ten_hoc_ky, $ghi_chu, $id_nam_hoc, $id_hoc_ky = null) {
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !hocky_valid_csrf()) {
        if ($_GET['req'] === 'add') {
            hocky_store_old_input('add', hocky_post_text('ten_hoc_ky'), hocky_post_text('ghi_chu'), filter_input(INPUT_POST, 'id_nam_hoc', FILTER_VALIDATE_INT));
        }
        if ($_GET['req'] === 'update') {
            hocky_store_old_input('update', hocky_post_text('ten_hoc_ky'), hocky_post_text('ghi_chu'), filter_input(INPUT_POST, 'id_nam_hoc', FILTER_VALIDATE_INT), filter_input(INPUT_POST, 'id_hoc_ky', FILTER_VALIDATE_INT));
        }
        hocky_redirect('csrf');
    }
        ...
        switch ($_GET['req']) {
            case 'add':
                ...
                if (!$id_nam_hoc || $id_nam_hoc < 1 || !$namhoc->namhoc__Get_By_Id($id_nam_hoc) || !hocky_valid_ten_hoc_ky($ten_hoc_ky) || !hocky_valid_ghi_chu($ghi_chu)) {
                    hocky_store_old_input('add', $ten_hoc_ky, $ghi_chu, $id_nam_hoc);
                    hocky_redirect('invalid');
                }
            ...
            case 'update':
                ...
                if (!$id_hoc_ky || $id_hoc_ky < 1 || !$id_nam_hoc || $id_nam_hoc < 1 || !$namhoc->namhoc__Get_By_Id($id_nam_hoc) || !hocky_valid_ten_hoc_ky($ten_hoc_ky) || !hocky_valid_ghi_chu($ghi_chu)) {
                    hocky_store_old_input('update', $ten_hoc_ky, $ghi_chu, $id_nam_hoc, $id_hoc_ky);
                    hocky_redirect('invalid');
                }
